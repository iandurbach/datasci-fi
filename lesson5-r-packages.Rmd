...
It's also worth noting that while `devtools` is here to stay, it’s being separated into more manageable pieces under the hood. A crucial part of the refactoring of `devtools` is the [usethis](http://usethis.r-lib.org/) package. It aims to automate many common package and analysis setup tasks (i.e. for both R package development projects and general R projects), and currently overs a number of potential workflow improvements if adopted directly in place of commonly used `devtools` helper functions. Refer to the `usethis` [setup article](http://usethis.r-lib.org/articles/articles/usethis-setup.html) for more info.
